...
def pendulum(
	...
	M = 0.5 # mass, in kg
	F_0 = 0.2 # constant friction coeficient, in N.s/m
	F_1 = 0.02 # linear friction coeficient, in N.s/m
	...
	def derivs(state, t):
		...
		W = -G * cost - (F_1*v + F_0)*np.sign(state[2])/M
		dxdy[2] = - RADIUS * w * w * cost - RADIUS * W * sint 
		dxdy[3] = - RADIUS * w * w * sint + RADIUS * W * cost
		return dxdy
